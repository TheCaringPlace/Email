...
import { useEmails } from "../../lib/hooks/emails";
...
export default function Index() {
  ...
  const { data: emails, mutate } = useEmails();
    ...
    <div className={"flex min-h-screen w-screen flex-col items-center justify-center gap-6"}>
      <div>
        {emails?.count && emails?.count > 0 ? (
          ...
          <motion.div key={"welcome"} initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }} className={"flex flex-col items-center justify-center gap-6"}>
            ...
            <div className={"w-full max-w-2xl space-y-3"}>
              ...
              <CodeBlock
                ...
                code={
                  {
                    javascript: `await fetch('${API_URI}/projects/${project.id}/send', {
  method: 'POST',
  body: JSON.stringify({
    to: "${user.email}",
...
requests.post(
    "${API_URI}/projects/${project.id}/send",
    ...
    json={
      ...
      "to": "${user.email}", 
                    ...
                    curl: `curl --location --request POST '${API_URI}/projects/${project.id}/send' \\
...
--data-raw '{"subject": "Your first email", "body": "Hello from Plunk!", "to": "${user.email}"}'`,
                    ...
                    PHP: `<?php
$client = new Client();
$request = new Request('POST', '${API_URI}/projects/${project.id}/send', ['Authorization' => 'Bearer ${keys?.secret}', 'Content-Type' => 'application/json'], '{
  "subject": "Your first email",
  "body": "Hello from Plunk!", 
  "to": "${user.email}", 
}');
...
url = URI("${API_URI}/projects/${project.id}/send")
...
response = https.request(request)`,
            ...
            <motion.button
              ...
              onClick={() => {
                toast.promise(
                  network.fetch(`/projects/${project.id}/send`, {
                    method: "POST",
                    body: {
                      subject: "Your first email",
                      body: "Hello from Plunk!",
