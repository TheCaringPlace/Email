...
import type { EventTrack, Template, TemplateCreate } from "@plunk/shared";
import { type ActionCreate, ActionSchemas, EventSchemas, TemplateSchemas } from "@plunk/shared";
import { AnimatePresence, motion } from "framer-motion";
import { useRouter } from "next/router";
import { useEffect, useState } from "react";
import { type FieldError, useForm } from "react-hook-form";
import { toast } from "sonner";
import { CodeBlock, Dropdown, Editor, FullscreenLoader, Modal, MultiselectDropdown, Toggle, Tooltip } from "../../components";
...
export default function Index() {
  ...
  const { data: keys } = useActiveProjectKeys();
  ...
  const triggerEvent = (data: EventValues) => {
    toast.promise(
      network.fetch(`/projects/${activeProject.id}/track`, {
        method: "POST",
        body: {
          event: data.event,
          email: user.email,
  ...
  const createTemplate = (data: TemplateCreate) => {
    toast.promise(
      network.fetch(`/projects/${activeProject.id}/templates`, {
        method: "POST",
        body: data,
      }),
      {
  ...
  const createAction = (data: ActionCreate) => {
    toast.promise(
      Promise.all([
        network.fetch<Template, ActionCreate>(`/projects/${activeProject.id}/actions`, {
          method: "POST",
          body: data,
        }),
      ]),
        ...
        return (
          ...
          >
            ...
            <div className={"max-w-2xl font-medium text-neutral-500"}>
              ...
              <p>In this 3 step tutorial, we'll help you set up your first email action so that you have an example on hand when you are ready to start building your own.</p>
            </div>

            <motion.button whileTap={{ scale: 0.9 }} whileHover={{ scale: 1.05 }} onClick={() => setStep(1)} className={"mt-6 rounded bg-neutral-800 px-12 py-4 text-sm font-medium text-white"}>
              Let's get started!
            </motion.button>
          </motion.div>
        );
        ...
        return events.length > 0 ? (
          ...
          >
            ...
            <p className={"font-medium text-neutral-500 sm:w-1/2"}>
              ...
              <span className={"rounded bg-neutral-50 px-2 py-0.5 font-mono text-neutral-600"}>{events.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())[0].name}</span>
              , you are now ready to create your first email template!
            </p>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} onClick={() => setStep(2)} className={"mt-4 rounded-md bg-neutral-800 px-10 py-3 text-sm font-medium text-white"}>
              Design an email
            </motion.button>
          </motion.div>
        ) : (
          <motion.div key={"event"} initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }}>
            <div className={"mx-auto my-6 max-w-xl text-center"}>
              <h2 className={"my-2 text-2xl font-bold"}>Track your first event</h2>
              <p className={"font-medium text-neutral-500"}>Actions start from events. You can call them whatever you want and send them from anywhere using an API call.</p>
            </div>
              ...
              <div className={"border-b border-neutral-100 p-4 sm:col-span-2 sm:border-b-0 sm:border-r-2"}>
                ...
                <div className={"mt-3 space-y-6"}>
                  ...
                  <CodeBlock
                    ...
                    code={
                      {
                        javascript: `await fetch('${API_URI}/projects/${activeProject.id}/track', {
  method: 'POST',
  body: JSON.stringify({
    event: "my-new-event",
    email: "${user.email}"
...
requests.post(
    "${API_URI}/projects/${activeProject.id}/track",
    ...
    json={
      ...
      "user": "${user.email}", 
                        ...
                        curl: `curl --location --request POST '${API_URI}/projects/${activeProject.id}/track' \\
--header 'Authorization: Bearer ${keys?.secret}' \\
--header 'Content-Type: application/json' \\
--data-raw '{"email": "hello@useplunk.com", "event": "new-project"}'`,

                        PHP: `<?php
$client = new Client();
$request = new Request('POST', '${API_URI}/projects/${activeProject.id}/track', ['Authorization' => 'Bearer ${keys?.secret}', 'Content-Type' => 'application/json'], '{
  "event": "my-new-event",
  "email": "${user.email}"
}');
...
url = URI("${API_URI}/projects/${activeProject.id}/track")

https = Net::HTTP.new(url.host, url.port)
https.use_ssl = true

request = Net::HTTP::Post.new(url)
request["Authorization"] = "Bearer ${keys?.secret}"
...
response = https.request(request)`,
      ...
      case 2:
        return (
          <motion.div key={"template"} initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }}>
            <div className={"mx-auto my-6 max-w-4xl text-center"}>
              <h2 className={"my-2 text-2xl font-bold"}>Design an email</h2>
              <p className={"font-medium text-neutral-500"}>Our templates are easy to write and automatically transformed into HTML that email clients understand.</p>
            </div>
            ...
            <form onSubmit={templateHandleSubmit(createTemplate)} className={"grid gap-6 sm:grid-cols-6"}>
              <div className={"sm:col-span-4"}>
                ...
                <div className="mt-1">
                  <input
                    autoComplete={"off"}
                    type={"text"}
                    className={"block w-full rounded-md border-neutral-300 transition ease-in-out focus:border-neutral-800 focus:ring-neutral-800 sm:text-sm"}
              ...
              <div className={"sm:col-span-2"}>
                ...
                <Dropdown
                  onChange={(t) => templateSetValue("templateType", t as "MARKETING" | "TRANSACTIONAL")}
                  values={[
                    { name: "Marketing", value: "MARKETING" },
                    { name: "Transactional", value: "TRANSACTIONAL" },
                  ]}
                  selectedValue={templateWatch("templateType") ?? ""}
                />
                <AnimatePresence>
                  {templateErrors.templateType?.message && (
                    <motion.p initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="mt-1 text-xs text-red-500">
                      {templateErrors.templateType.message}
                    </motion.p>
                  )}
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.9 }}
                className={"my-6 ml-auto flex items-center gap-x-0.5 rounded-md bg-neutral-800 px-10 py-2.5 text-center text-sm font-medium text-white sm:col-span-6"}
              >
        ...
        return actions.length > 0 ? (
          ...
          >
            ...
            <motion.button
              ...
              onClick={async () => {
                toast.promise(
                  network.fetch<never, EventTrack>(`/projects/${activeProject.id}/track`, {
                    method: "POST",
                    body: {
          ...
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }} key={"action"}>
            <div className={"mx-auto my-6 max-w-2xl text-center"}>
              <h2 className={"my-2 text-2xl font-bold"}>Creating your first action</h2>
              <p className={"font-medium text-neutral-500"}>Actions tie together events and templates, they automate your email workflows.</p>
            </div>
            <form onSubmit={actionHandleSubmit(createAction)} className="grid gap-4 space-y-6 pb-6 sm:grid-cols-2">
              ...
              <AnimatePresence>
                {advancedSettings && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} key={"advanced"} className={"sm:col-span-2"}>
                    <div>
                      <label htmlFor={"template"} className="block text-sm font-medium text-neutral-700">
                    ...
                    <div className={"mt-8 sm:col-span-2"}>
                      <Toggle
                        title={"Run once"}
                        description={actionWatch("runOnce") ? "This action will only run once for each contact." : "This action will run each time the required events are triggered."}
                        toggled={actionWatch("runOnce") ?? false}
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.9 }}
                className={"ml-auto flex items-center gap-x-0.5 rounded-md bg-neutral-800 px-10 py-2.5 text-center text-sm font-medium text-white sm:col-span-2"}
              >
                <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
                  <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 5.75V18.25" />
                  <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M18.25 12L5.75 12" />
            ...
            <nav aria-label="Progress">
              <ol className="space-y-4 md:flex md:space-x-8 md:space-y-0">
                <li className="md:flex-1">
                  <span
                    className={`${step >= 0 ? "border-neutral-800" : "border-neutral-200"} group flex flex-col border-l-4 py-2 pl-4 transition md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4`}
                  >
                ...
                <li className="md:flex-1">
                  <span
                    className={`${step >= 1 ? "border-neutral-800" : "border-neutral-200"} group flex flex-col border-l-4 py-2 pl-4 transition md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4`}
                  >
                ...
                <li className="md:flex-1">
                  <span
                    className={`${step >= 2 ? "border-neutral-800" : "border-neutral-200"} group flex flex-col border-l-4 py-2 pl-4 transition md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4`}
                  >
                ...
                <li className="md:flex-1">
                  <span
                    className={`${step >= 3 ? "border-neutral-800" : "border-neutral-200"} group flex flex-col border-l-4 py-2 pl-4 transition md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4`}
                  >
